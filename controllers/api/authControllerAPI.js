    ...
    authenticate: function(req, res, next){
        Usuario.findOne({ email: req.body.email}, function(err, userInfo){
            if(err){
                ...
                if(userInfo === null){ return res.status(401).json({status:"error", message: "Password o email invalidos", data: null}); }
                if(userInfo != null && bcrypt.compareSync(req.body.password, userInfo.password)){
                    ...
                    res.status(401).json({status: "error", message: "Password o email invalidos", data: null});
                }
            }
        })
    },
